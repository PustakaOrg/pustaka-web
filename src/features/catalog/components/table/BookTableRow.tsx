...
const BookTableRow = ({book,columnVisibility,isSelected,onSelect,onAction}:BookTableRowProps) => {
return (
    <TableRow>
      <TableCell>
        <Checkbox
      ...
      {columnVisibility.title && <TableCell className="font-medium">{book.title}</TableCell>}
      {columnVisibility.isbn && <TableCell className="font-mono text-sm">{book.isbn}</TableCell>}
      {columnVisibility.author && <TableCell>{book.author?.fullname}</TableCell>}
      {columnVisibility.publisher && (
        <TableCell>
          <div>
            <div className="font-medium">{book.publisher?.name}</div>
            <div className="text-sm text-muted-foreground">{book.publisher?.city}</div>
          </div>
        </TableCell>
      )}
      ...
      {columnVisibility.pages && <TableCell className="text-right">{book.pages}</TableCell>}
      {columnVisibility.year && <TableCell className="text-right">{book.publish_year}</TableCell>}
